...
import { ArrowRight, Loader2, CheckCircle, Tag } from "lucide-react";
...
export function GuestCheckoutDialog({ open, onOpenChange, product }: GuestCheckoutProps) {
  ...
  const handleApplyPromo = async () => {
    ...
    if (data.applies_to_type && data.applies_to_type !== product.type) {
      toast.error("Ce code ne s'applique pas à ce type de produit");
      setPromoApplied(null);
      setCheckingPromo(false);
      return;
    }
    ...
    // Check max uses
    if ((data as any).max_uses > 0 && (data as any).times_used >= (data as any).max_uses) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (needsDelivery && !deliveryAddress.trim()) {
      toast.error("L'adresse de livraison est requise pour un livre");
      return;
    }
    setLoading(true);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {needsDelivery && (
            <div className="space-y-3 rounded-lg border border-border p-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="delivery-location" className="text-sm">Localisation / point de repère</Label>
                <Input
                  id="delivery-location"
                  value={deliveryLocation}
                  onChange={(e) => setDeliveryLocation(e.target.value)}
                  placeholder="Ex: Près de la pharmacie X, en face de…"
                />
              </div>
            </div>
          )}
